...
public class ApiController {
    @Autowired
    protected TCareWebApplication application;
    ...
    private static final Map<Class, Object> commandServiceMap = new HashMap<>();
    ...
    public static Environment getEnvironment() {
        if (environment == null) {
            environment = TCareWebApplication.global.getEnvironment();
        }
        return environment;
    }
    ...
    public static <T> T getService(Class<T> clazz) {
        ...
        if (objService == null) {
            T targetService = TCareWebApplication.global.getBean(clazz);
    ...
    public static <T> T getCommandService(Class<T> clazz) {
        Object objService = commandServiceMap.get(clazz);
        if (objService == null) {
            T targetService = TCareWebApplication.global.getBean(clazz);
            commandServiceMap.put(clazz, targetService);
            objService = targetService;
        }
        return (T) objService;
    }
    ...
    public static String getMimeType(String fileExt) {
        ...
        if (fileExt.equalsIgnoreCase("webp"))
            return "image/webp";
    ...
    public static String getEncodedFileSeq(String fileUrl) {
        try {
    ...
    public static String getHyphenFormat(String source) {
    ...
    public static String getDownloadFileName(String browser, String fileName) {
    ...
    public static Locale getServerLocale() {
        if (serverLang == null || serverLang.isEmpty()) {
            serverLang = getEnvironment().getProperty("tcare.server.lang");
            if (serverLang == null || serverLang.isEmpty())
                serverLang = "EN";
        }
        if (serverLang.equalsIgnoreCase("KO")) return Locale.KOREAN;
        if (serverLang.equalsIgnoreCase("JA")) return Locale.JAPANESE;
        if (serverLang.equalsIgnoreCase("ZH-CN")) return Locale.SIMPLIFIED_CHINESE;
        if (serverLang.equalsIgnoreCase("ZH-TW")) return Locale.TRADITIONAL_CHINESE;
        return Locale.ENGLISH;
    }

    public static Locale getUserLocale(String lang) {
        if (lang == null || lang.isEmpty()) lang = "EN";
        if (lang.equalsIgnoreCase("KO")) return Locale.KOREAN;
        if (lang.equalsIgnoreCase("JA")) return Locale.JAPANESE;
        if (lang.equalsIgnoreCase("ZH-CN")) return Locale.SIMPLIFIED_CHINESE;
        if (lang.equalsIgnoreCase("ZH-TW")) return Locale.TRADITIONAL_CHINESE;
        return Locale.ENGLISH;
    }
}
